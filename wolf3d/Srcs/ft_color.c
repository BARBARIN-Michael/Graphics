...
/*   Updated: 2015/03/08 16:39:49 by mbarbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <ft_wolf.h>

static t_rgb	*ft_setup_color()
{
	t_rgb	rgb[17];
	//Nord
	rgb[0] = (t_rgb) {.r = 0x57, .g = 0x2B, .b = 0x06};
...
{
	static t_rgb *rgb = (void *)0;

	if (rgb == (void *)0)
		rgb = ft_setup_color();
	rgb = (t_rgb) {.r = rgb.r / mod, .g = rgb.g / mod, .b = rgb.b / mod};
	return (map_elem >= 0 && map_elem < 16 ? rgb[map_elem] : rgb[0]);
}
